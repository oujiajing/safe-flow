...
describe('useAccessStore', () => {
  ...
  it('does not persist login identity across page launches', () => {
    const source = readFileSync(resolve(__dirname, 'access.ts'), 'utf8');
    const persistedFields = source.match(/pick:\s*\[([\s\S]*?)\]/)?.[1] ?? '';

    expect(persistedFields).not.toContain("'accessToken'");
    expect(persistedFields).not.toContain("'refreshToken'");
    expect(persistedFields).not.toContain("'accessCodes'");
  });
