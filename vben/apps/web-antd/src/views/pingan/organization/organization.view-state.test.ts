...
describe('organization relationship view state', () => {
  ...
  it('renders the demo group graph with only third-level company cards', () => {
    ...
    expect(relation.graphView?.groups.map((group) => group.parent.node.key)).toEqual([
      'company-muqian',
      ...
      'company-guangsheng-storage',
      'company-jinyue-muqian',
      'company-fanao',
    ]);
    expect(
  ...
  it('expands a clicked third-level company under the group graph without switching roots', () => {
    ...
    expect(relation.graphView?.mode).toBe('GROUP');
    expect(relation.rootNode?.title).toBe('演示安全集团');
    expect(relation.graphView?.chain.map((item) => item.node.title)).toEqual([
      '演示安全集团',
      '演示安全集团',
    ]);
    expect(
      relation.graphView?.groups
        .find((group) => group.parent.node.key === 'company-mining-invest')
        ?.children.map((item) => item.node.key),
    ).toEqual([
      'company-meizhou-jiasheng',
      'company-heyuan-guyun',
      'company-botai',
      'company-chaoan-liyuan',
      'company-guangdong-metallurgy',
      'company-heshangtian',
    ]);
    expect(
      relation.graphView?.groups
        .filter((group) => group.parent.node.key !== 'company-mining-invest')
  ...
  it('expands all third-level companies in the group graph when multiple ids are provided', () => {
    ...
    expect(
      relation.graphView?.groups
        .find((group) => group.parent.node.key === 'company-muqian')
        ?.children.map((item) => item.node.key),
    ).toEqual(['company-yuancheng']);
    expect(
      relation.graphView?.groups
        .find((group) => group.parent.node.key === 'company-mining-invest')
        ?.children.map((item) => item.node.key),
    ...
    expect(
      relation.graphView?.groups
        .find((group) => group.parent.node.key === 'company-south-storage')
        ?.children.map((item) => item.node.key),
    ).toEqual([
      'company-south-storage-transport',
      'company-foshan',
      'company-changzhou-south-storage',
    ]);
  });
