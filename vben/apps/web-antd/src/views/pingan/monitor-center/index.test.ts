...
describe('monitor center page', () => {
  ...
  it('filters the large-screen dashboard by date range and approved companies', () => {
    ...
    expect(screenSource).not.toContain('Demo Works Company');
  });
  ...
  it('loads team analysis from the real aggregate API on the seeded validation date', () => {
    ...
    expect(teamSource).toContain('teamAnalysisStartDate.setDate');
    expect(teamSource).toContain('dayjs(teamAnalysisStartDate)');
    expect(teamSource).toContain('dayjs(teamAnalysisEndDate)');
