...
describe('pre-shift-meeting data', () => {
  it('exposes the demo organization tree used by the data map', () => {
    expect(organizationTree[0]?.title).toBe('演示安全集团');
    expect(organizationTree[0]?.children?.[0]?.title).toBe('演示安全集团');
  });
  ...
  it('contains the data-map organizations shown in the first prototype page', () => {
    ...
    expect(titles).not.toContain('Demo Works Company');
    expect(titles).not.toContain('Demo HarborA6 / 4000-MQXM001-001');
  });
  ...
  it('keeps the fixed demo company as a data-map leaf while preserving its id', () => {
  ...
  it('flattens organization nodes for selectable form fields', () => {
    ...
    expect(options).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ label: '演示公司', value: 4 }),
        expect.objectContaining({ label: '演示单位', value: 8 }),
      ]),
    );
  });
  ...
  it('returns rows matching the selected team node', () => {
    ...
    expect(rows.map((item) => item.company)).toContain('演示公司');
  });
});
