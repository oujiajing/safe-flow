...
class SystemContentProfileApiTest {
  ...
  void managesContentProfilesAndRejectsDuplicateOrganizations() throws Exception {
    ...
    JsonNode list = getJson("/api/system/content-profiles?keyword=幕墙更新&status=ACTIVE", token).path("data");
    ...
    assertThat(
        ...
        .isEqualTo(id);
    assertThat(getJson("/api/system/content-profiles?keyword=幕墙更新", token).path("data").path("total").asInt())
        .isZero();
