...
class ThreeCheckRecordApiTest {
  ...
  void approvedQuickShotCopiesUploadedAttachmentUrlToOrderItemMedia() throws Exception {
    ...
    String beforePhoto = detail.path("items").get(0).path("beforePhoto").asText();
    assertThat(beforePhoto)
        .startsWith("/api/attachments/" + uploadResponse.path("id").asText() + "/content?");
    assertThat(beforePhoto).contains("expires=", "signature=");
    ...
    JsonNode fallbackDetail =
        getJson(
                ...
                token)
            .path("data");
    String fallbackBeforePhoto = fallbackDetail.path("items").get(0).path("beforePhoto").asText();
    assertThat(fallbackBeforePhoto)
        .startsWith("/api/attachments/" + uploadResponse.path("id").asText() + "/content?");
    assertThat(fallbackBeforePhoto).contains("expires=", "signature=");
  }
