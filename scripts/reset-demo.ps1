...
if (-not $ConfirmDemo) { throw "Pass -ConfirmDemo to reset only the local Portfolio Demo account." }
...
$result = & java -cp "$temp;$($securityJar.FullName);$($postgresJar.FullName);$($loggingJar.FullName)" DemoAccountCli reset $Username $dbUrl $dbUser
if ($LASTEXITCODE -ne 0) { throw "Demo account reset failed; no password was displayed as a valid credential." }
Write-Output "Reset local Demo account: $Username"
